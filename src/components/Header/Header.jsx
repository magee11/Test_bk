import React, { useEffect, useState } from "react";
import "./header.css";
import { useSelector, useDispatch } from "react-redux";
import { IsOpen, IsLogedIn } from "../../redux/app/counter/LoginPop";
import { Link } from "react-router-dom";
import { useNavigate } from "react-router-dom";
import Login from "../Login/Login";

function Header() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const cartItems = useSelector((state) => state.counter.cartItem);
  const openData = useSelector((state) => state.popup.popUpIsOpen);
  const LoginData = useSelector((state) => state.popup.Login);
  const [Theme, setTheme] = useState("delivery");

  const toggleChangeButton = () => {
    if (Theme == "dine-in") {
      setTheme("delivery");
    } else {
      setTheme("dine-in");
    }
  };
  //   document.addEventListener("mousedown", handler);

  const cartNav = () => {
    if (cartItems) {
      navigate("/cart");
    } else {
      alert("Your Cart is Empty!");
    }
  };
  return (
    <div className="header">
      <div className="headerWrapper">
        <Link to={"/"}>
          <div>
            <img
              className="logo"
              src="data:image/png;base64,"
              alt=""
            />
          </div>
        </Link>
        <div className="UserControls">
          <p
            className="title"
            style={{ color: Theme == "dine-in" ? "Brown" : "grey" }}
          >
            Delivery
          </p>
          <div className="toggle button">
            <div
              className="toggle button"
              onChange={() => {
                toggleChangeButton();
              }}
            >
              <label class="switch">
                <input type="checkbox" />
                <span class="slider round"></span>
              </label>
            </div>
          </div>
          <p
            className="title"
            style={{ color: Theme == "delivery" ? "#ee7700" : "grey" }}
          >
            Dine-in/Take Away
          </p>

          <div className="SearchBar">
            {Theme == "delivery" ? (
              <span className="searchText">Tap to pic your loaction </span>
            ) : (
              <span className="searchText">Indiranagar, Bangalore</span>
            )}
          </div>
        </div>
        <div className="NavMenu">
          <ul className="sideContent">
            <li className="NavItems">
              <Link to={"/find-restaruent"}>
                <img
                  className="NavIcons"
                  src="https://www.burgerking.in/static/media/dine-in-black.c32e1cdd.svg"
                  alt=""
                />
                <a href="">Nearlby By</a>
              </Link>
            </li>
            <li className="NavItems">
              <Link to={"/kingdeals"}>
                <img
                  className="NavIcons"
                  src="https://www.burgerking.in/static/media/offers-inactive.81dcb21f.svg"
                  alt=""
                />
                <a href="">King Deals</a>
              </Link>
            </li>
            <li className="NavItems">
              {LoginData == true ? (
                <Link to={"/profile"}>
                  <div
                    className="LoginClick"
                   
                  >
                    <img
                      className="NavIcons"
                      src="https://www.burgerking.in/static/media/login-inactive.de941b20.svg"
                      alt=""
                    />
                    <p> Profile</p>
                  </div>
                </Link>
              ) : (
                <Link to={"/"}>
                  <div
                    className="LoginClick"
                    onClick={() => {
                      dispatch(IsOpen());
                    }}
                  >
                    <img
                      className="NavIcons"
                      src="https://www.burgerking.in/static/media/login-inactive.de941b20.svg"
                      alt=""
                    />
                    <p>Login</p>
                  </div>
                </Link>
              )}
            </li>
            <li className="NavItems">
              <div
                className="cartIconsLables"
                onClick={() => {
                  cartNav();
                }}
              >
                <img
                  className="NavIcons"
                  src="https://www.burgerking.in/static/media/cart.3ff203fa.svg"
                  alt=""
                />
                <span className="cartItems">{cartItems}</span>
                <a href="">Cart</a>
              </div>
            </li>
          </ul>
          <div className="search">
            <img
              src="https://www.burgerking.in/static/media/search.03064934.svg"
              alt=""
            />
          </div>
        </div>
      </div>
      <div>{openData && <Login />}</div>
    </div>
  );
}

export default Header;
